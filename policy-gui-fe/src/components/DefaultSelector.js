...
import '../styles/DefaultSelector.css';
...
function DefaultSelector({ defaults, onSelectDefault, currentFormData }) {
  ...
  const handleDefaultSelect = (defaultKey) => {
    ...
    if (defaultConfig && defaultConfig.values) {
      // Merge the default values with current form data
      const mergedData = { ...currentFormData, ...defaultConfig.values };
      onSelectDefault(mergedData);
